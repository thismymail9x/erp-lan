...
-- 2026-03-30: PHÂN HỆ QUẢN LÝ KPI VÀ THƯỞNG QUY TRÌNH
-- --------------------------------------------------------
ALTER TABLE `workflow_template_steps` ADD COLUMN `kpi_reward` DECIMAL(15,2) DEFAULT 0 COMMENT 'Mức thưởng/KPI cho bước này';
ALTER TABLE `case_steps` ADD COLUMN `kpi_reward` DECIMAL(15,2) DEFAULT 0 COMMENT 'Ghi nhận mức thưởng định mức từ bản mẫu';
ALTER TABLE `case_steps` ADD COLUMN `overdue_notified` TINYINT(1) DEFAULT 0 COMMENT 'Đã gửi thông báo quá hạn chưa';

-- 2026-03-30: FIX FOREIGN KEY CONSTRAINT FOR VOLUNTARY DOCUMENTS
-- Nghiêm cấm xóa bản ghi hoặc vi phạm FK khi không chọn vụ việc/khách hàng
ALTER TABLE `documents` MODIFY COLUMN `case_id` INT(11) UNSIGNED NULL COMMENT 'Vụ việc liên quan';
ALTER TABLE `documents` MODIFY COLUMN `customer_id` INT(11) UNSIGNED NULL COMMENT 'Khách hàng sở hữu tài liệu';
ALTER TABLE `documents` MODIFY COLUMN `step_id` INT(11) UNSIGNED NULL COMMENT 'Bước thực hiện';
